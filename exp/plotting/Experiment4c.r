...
pdf(file="plots/Experiment4_Prop50.pdf", width=3.7, height=4.0, family="serif", pointsize=14)
...
for (i in (numOfPoints - lastX +1):(numOfPoints)) {
    file_name <- paste0("results/Experiment4_times_OnlyMaxScore_Prop50_", (i +10), ".dat")
...
text(x = lastXPoints[floor(length(lastXPoints)/5)*4], y = 150, 
     labels = expression(paste(frac(abs(italic(" addedX ")), abs(italic(" totalX "))), " = 0.5")), cex = 0.5)
...
write.table(output_data, file = "results/allData/Experiment4_Prop50_AllData.csv", sep = ",", row.names = FALSE, col.names = TRUE)
