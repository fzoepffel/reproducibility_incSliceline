...
pdf(file="plots/Experiment2a.pdf", width=3.7, height=4.0, family="serif", pointsize=14)
...
plot(tail(points, lastX), tail(data[, 1], lastX),  
    main="100x Increasing Input Size, Same Proportions",
    cex.main=0.7,    
    type="o",           
    pch=19, 
    cex=0.1,
...
lines(points, data[, 2], type = "o", cex=0.1, col = plot_colors[2], lty = 2, pch = 19)
...
write.table(output_data, file = "results/allData/Experiment2a_AllData.csv", sep = ",", row.names = FALSE, col.names = TRUE)
